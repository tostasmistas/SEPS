...
<<<<<<< HEAD
É de salientar que no \textit{scrambler} e no \textit{descrambler} existe um \textit{delay} de N \textit{bits}, sendo a saída do \textit{descrambler} não igual á entrada do \textit{scrambler}, mas sim igual à entrada deslocada de N:
=======
...
De seguida, é apresentado o sinal de saída do \textit{descrambler}, $b'_n$ e o sinal de entrado do \textit{scrambler}, $b_n$. Pode verificar-se que os sinais são iguais, pois a implementação da conexação dos dois circuitos foi realizada com um \textit{delay} de N igual \texttt{0}.

\begin{figure}[H]
	\centering
	\includegraphics[keepaspectratio=true, scale=0.40]{exps/DescramblerBnVsBnlinha}
	\caption{Imagem que demonstra o sinal de entrado do \textit{scrambler}, a amarelo, e do sinal de saída do \textit{descrambler}, a verde.}
	\vspace{-0.8em}
\end{figure}


\todo{fazer a ponte para esta parte do projecto}

Pretende-se implementar um NCO (\textit{numerically controlled oscillator}) controlado pelo sinal de erro $\varepsilon (t)$ com a seguinte característica:
...
sendo que $f_{ol}$ trata-se da frequência livre do oscilador, que deve ser definida como sendo igual à frequência da portadora do sinal BPSK, ou seja, $f_{ol} = f_0$ e que $\varepsilon (n)$ se trata da saída do filtro \textit{Loop}, que será implementado posteriormente. Assim, considerando o gerador de uma onda sinusoidal implementado na primeira parte do projecto, que usa uma 32 valores de meio período da função seno, é possível implementar este oscilador de forma idêntica, tendo em atenção o valor de $f_ol$ e de $K_0$, que tal como no oscilador implementado anteriormente, tem o valor de $1/4$. É ainda pretendido que seja gerada uma outra saída em quadratura, o que é possível lendo a LUT 16 valores depois. É apresentado o excerto de código que implementa o oscilador, permitindo que sejam geradas as duas saídas em quadratura.
...
\todo{LEGENDA DA FIGURA - Qual é o seno e o coseno? Eu sei que é igual pormos que o verde é um ou o outro, era só para saber se tinham ideia}
...
Quando se analisa as figuras anteriores verifica-se que são bastante semelhantes, não havendo diferenças perceptíveis a ``olho nu''. De facto, quando se multiplica as duas sinusoides CONTINUAR AQUI
