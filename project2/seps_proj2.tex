...
A implementação do \textit{descrambler} tem a função de obter a sequência de \textit{bits} correcta para o que PC. Assim sendo, o circuito do \textit{descrambler} tem que conseguir obter toda a sequência de \textit{bits} que entra no \textit{scrambler}.
Anteriormente foi referido que o \textit{scrambler} segue a seguinte expressão polinomial:
\begin{equation}
P(x)~= ~1~+~x^{-6}~+~x^{-7}
\end{equation}
Partindo desta equação consegue obter a expressão de recuperação do sinal de entrada do \textit{scrambler}:

\vspace{-3mm}
\begin{equation}
b_n'~= ~e_n~+~e_{n-6}~+~e_{n-7};
\end{equation}  
...
\todo{tabela das bandas com tudo junto para se comparar}
...
De seguida, faz-se uma análise do transitório do processo de aquisição do \textit{loop}. CONTINUAR AQUI
