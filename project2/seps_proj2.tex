...
Pode-se verificar na figura anterior que o sinal de entrada do filtro representa a multiplicação do sinal BPSK com o sinal de saída do oscilador. Como já foi referenciado anteriormente, a multiplicação de duas sinusoides origina a soma de duas componentes, uma com a diferença das frequências e outra com a soma das frequências de cada um dos sinais sinusoidais. Como o sinal BPSK tem como frequência de portadora o mesmo valor da frequência do sinal de saída do oscilador, a primeira componente seria o sinal $s(t)cos(2\pi t(f_0-f_{ol})$ e a segunda componente $s(t)\cos(2\pi t(f_0+f_{ol}))$. 
A primeira componente corresponde ao envolvente do sinal de entrada do filtro e a segunda componente corresponde às oscilações que se visualização no sinal. 
A função do filtro de dados é retirar as oscilações do sinal de entrada obtendo assim a sua envolvente, esta conclusão pode ser visualizada na Figura 39.
