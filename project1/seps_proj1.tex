...
Com a LUT declarada, usou-se o excerto de código seguinte para poder gerar a portadora de frequência $f_0$ a 4 kHz. O código encontra-se no \textit{loop} da rotina principal de forma a obter um valor da LUT a cada 16 kHz, incrementando a variável de indexação da LUT a cada passagem do \textit{loop}.
...
\todo{porque a mascara com os 3 bits menos significativos? nunca falamos sequer do sinal sine\_i - o que representa? - teddy}

\todo{y = (32767*y)>>15 ; - linha de codigo que nunca é explicada - teddy}
...
Com o modulador criado procede-se à fase de testes. Sabe-se que o sinal modulado do tipo BPSK, é um sinal com ritmo de transmissão de 1 kbps, $d_n$, modulado por uma portadora de 4 kHz. Assim, existe uma inversão de fase a cada 8 ciclos. \todo{nao se percebe porquê - teddy}

Analisando a figura seguinte verifica-se que o modulador está a funcionar correctamente, invertendo a fase no zero da portadora e a cada 8 ciclos.
